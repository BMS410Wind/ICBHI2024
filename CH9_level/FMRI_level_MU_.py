...
for i in x_train:
    ...
    npy_data = npy_data[(1,10,14,17,42,43,54,55,86,87,104,105,106,107,134,135,140,141,142,143,150,151,158,159,182,183,190,191,194,195,202,203),:]
...
inputA = Input(shape=(25,32))
inputB = Input(shape=(25,32))
inputC = Input(shape=(25,32))

#F = SE_Block(inputA)
F_shortcut1 = inputA

F = Conv1D(filters=32, kernel_size=(3), strides=1,padding='same', activation='relu',kernel_initializer='he_uniform',kernel_regularizer=regularizers.l2(0.01))(inputA)
F = SE_Block(F)
...
specificity_score(y_test_, pred_list, average='macro')
#%%
sensitivity_score(y_test_, pred_list, average='macro')
#%%
#%%outside_test
counter = 0
model.load_weights("./models/best.h5")
